...
def validate_pdf_content(text):
    ...
    # Términos médicos inglés + español
    medical_terms = [
        # inglés
        'blood', 'test', 'report', 'laboratory', 'lab', 'patient',
        'specimen', 'reference range', 'analysis', 'results',
        'medical', 'diagnostic', 'hemoglobin', 'wbc', 'rbc',
        'platelet', 'glucose', 'creatinine',

        # español
        'sangre', 'análisis', 'laboratorio', 'paciente',
        'resultado', 'hemoglobina', 'plaquetas',
        'glucosa', 'creatinina', 'hematocrito',
        'leucocitos', 'eritrocitos', 'colesterol',
        'triglicéridos', 'urea'
    ]
    ...
    # Si el PDF tiene bastante texto, aceptarlo igual
    if len(text) > 500:
        return True, None

    return False, "The uploaded file doesn't appear to be a medical report."